...
**核心问题**：ANALYZE_REPORT.md 仍是 DRAFT 且有 Confirmation Needed，但 patch-guard 仍生成 LOCKED 文件。
...
调用 `/patch-guard` 后第一响应必须包含 Rules Check：
...
修复显式调用优先级规则，防止 AI 以 "简单问题" 为由绕过 patch-guard。

**核心问题**：用户显式调用 `/patch-guard` 后，AI 仍然以 "问题简单" "只改两行" 等理由跳过 Analyze，直接修改代码。
...
**只要用户消息中显式出现 `/patch-guard`，就必须进入流程。**
...
| File | Change |
|------|--------|
| SKILL.md | 完全重写，单入口 `/patch-guard`，自动阶段推进，Patch ID 机制，新目录结构 |
| README.md | 更新为单入口说明，新目录结构，移除手动阶段命令 |
| docs/QUICK_START.md | 更新为 `/patch-guard` 入口，Patch ID 规则，新目录结构 |
...
Problem-Focused Governance 修正，让 patch-guard 不只检查流程合规，还要检查原始问题是否被修复。
